...
# smiles_back: list of SMILES to be evaluated, target_smi: one target SMILES
...
def form_joint_path(starting_selfie_chars, struct_2_selfie_chars, struct_3_selfie_chars, triplet):  
    ''' Create a generalized chemical path starting from the molecule 'starting_selfie_chars' (provided as a list of chars)
        to 'struct_2_selfie_chars' & 'struct_3_selfie_chars'
...
#     triplets_all = pickle.load(fp) # [(smi1, smi2, smi3), (smi1, smi2, smi3), ...] len=100

for iter in range(100): # ligandsのグループ作成をrandom.shuffleで10回繰り返して生成
    sdf_file_path = 'CBLB_inhibitors_vsF.sdf'
    suppl = Chem.SDMolSupplier(sdf_file_path)
    ligands = [Chem.MolToSmiles(mol) for mol in suppl if mol is not None]
    random.shuffle(ligands)
    # idx_list = np.random.randint(0, len(ligands), len(ligands)).tolist()
    collect_unfilt = {}
    collect_filt   = {}
    triplets_all = []
    for i in range(int(len(ligands)/3)):
        triplets_all.append((ligands[3*i], ligands[3*i + 1], ligands[3*i + 2]))
    ...
    for trip_id, triplet in enumerate(triplets_all): 
        ...
        for iter_ in tqdm(range(num_paths),desc='num_paths'): 
            ...
            # tripletのtupleの中身をrandomizeする
            
            random_choice = np.random.choice([i for i in range(len(triplet))], 3, replace=False)
            # 0, 1, 2 の数字の中からrandomに取り出す操作を3回やる, replace=Falseなので重複はナシ
        ...
        for k,smi in enumerate(ALL_PATHS):
            ...
            if sa_score < 3.5 and qed_score > 0.6:
                fp_mol = get_ECFP4(mol)
                for ligand in suppl:
                    fp_ligand = get_ECFP4(ligand)
                    similarity = TanimotoSimilarity(fp_mol, fp_ligand)
                    if similarity > 0.7:
                        flag = True
                        break
                if flag == True:
                    continue
                mol, smi, _ = sanitize_smiles(smi)
                smi_canon = Chem.MolToSmiles(mol, kekuleSmiles=False)
                better_smi.append((smi_canon, k, sa_score, qed_score))
                
                
        filtered_smiles = [x[0] for x in better_smi]
        filtered_scores = [ALL_SIM[x[1]] for x in better_smi]
        filtered_sascores = [x[2] for x in better_smi]
        collect_filt[trip_id] = [filtered_smiles, filtered_scores, filtered_sascores]
        ...
        #     print('None')
    num_sample = 0
    for smis in collect_filt.values():
        num_sample += len(smis)
        
    # Saving the results:      
    print(f'iter{iter}: num of filterd compounds = {num_sample}')
    with open(f'./BEST_MEDN_2/medn_filt_{iter}.pickle', 'wb') as fp:   #Pickling
